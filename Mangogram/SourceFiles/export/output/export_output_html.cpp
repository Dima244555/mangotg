...
QByteArray MakeLinks(const QByteArray &value) {
	const auto domain = QByteArray("https://mangogram.org/");
	auto result = QByteArray();
	auto offset = 0;
	while (true) {
...
auto HtmlWriter::Wrap::pushMessage(
	...
	if (v::is<UnsupportedMedia>(message.media.content)) {
		return { info, pushServiceMessage(
			...
			"of Mangogram Desktop. Please update the application.") };
	}
	...
	const auto serviceText = v::match(message.action.content, [&](
		...
		return serviceFrom + " joined Mangogram";
		...
		return data.unclaimed
			? ("This is an unclaimed Mangogram Premium for "
				+ NumberToString(data.days)
				+ (data.days > 1 ? " days" : " day")
				+ " prize in a giveaway organized by a channel.")
			: data.viaGiveaway
			? ("You won a Mangogram Premium for "
				+ NumberToString(data.days)
				+ (data.days > 1 ? " days" : " day")
				+ " prize in a giveaway organized by a channel.")
			: ("You've received a Mangogram Premium for "
				+ NumberToString(data.days)
				+ (data.days > 1 ? " days" : " day")
				+ " gift from a channel.");
	}, [&](const ActionGiveawayLaunch &data) {
		return serviceFrom + " just started a giveaway "
			"of Mangogram Premium subscriptions to its followers.";
		...
		return !data.winners
			...
			? "Some winners of the giveaway were randomly selected by "
				"Mangogram and received their prize."
			: (!data.credits && data.unclaimed)
			? "Some winners of the giveaway were randomly selected by "
				"Mangogram and received private messages with giftcodes."
			: (data.credits && !data.unclaimed)
			? NumberToString(data.winners) + " of the giveaway was randomly "
				"selected by Mangogram and received their prize."
			: NumberToString(data.winners) + " of the giveaway was randomly "
				"selected by Mangogram and received private messages with "
				"giftcodes.";
		...
		return serviceFrom
			...
			+ (data.amount.ton() ? " TON." : " Mangogram Stars.");
	...
	if (data.credits > 0) {
		...
		result.append("<b>"
			...
			+ SerializeString((data.quantity > 1)
				? "Mangogram Premium Subscriptions"
				: "Mangogram Premium Subscription")
...
QByteArray HtmlWriter::Wrap::pushGiveaway(
	...
	result.append(
		...
		+ " was randomly selected by Mangogram.");
	result.append(popTag());
