		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"MangoTG.exe") && exeName != L"MangoTG.exe") {
	...
	for (size_t i = 0; i < from.size(); ++i) {
		...
		do {
			...
			if (!copyResult) {
				...
				WCHAR errMsg[2048];
				wsprintf(errMsg, L"Failed to update Mangogram :(\n%s is not accessible.", tofname.c_str());
...
void updateRegistry() {
	if (versionNum && versionNum != 0x7FFFFFFF) {
		...
		HKEY rkey;
		LSTATUS status = RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{5C1D1642-EA7A-4BB9-8D68-5F898F161272}_is1", 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &rkey);
					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								const WCHAR *appURL = L"https://github.com/";
...
static const WCHAR *_programName = L"Mangogram Desktop"; // folder in APPDATA, if current path is unavailable for writing
