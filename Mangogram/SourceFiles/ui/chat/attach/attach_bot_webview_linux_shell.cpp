...
QByteArray InstallScript(const QString &shellToken) {
	...
	script += "if (window === window.top"
		" && !window.MangogramDesktopShell"
		" && !window.MangogramDesktopShellInstalling) {"
		"window.MangogramDesktopShellInstalling = true;"
		"try {"
		"if (!document.head) {"
	...
	script += pageJs;
	script += "} finally {"
		"window.MangogramDesktopShellInstalling = false;"
		"}"
		"}";
	return script;
}
...
QByteArray MethodCallScript(
	...
	script += "if (window.MangogramDesktopShell"
		" && window.MangogramDesktopShell.";
	script += method;
	script += ") { window.MangogramDesktopShell.";
	script += method;
	script += "(";
	script += payload;
	script += ", ";
...
QByteArray EventScript(
	...
	auto script = QByteArray();
	script += "if (window.MangogramDesktopShell) {"
		"window.MangogramDesktopShell.nativeEvent(";
	script += eventValue;
	script += ", ";
	script += payload;
	script += ", ";
