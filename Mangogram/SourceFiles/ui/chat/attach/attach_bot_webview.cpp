...
[[nodiscard]] QString ExternalShellTopUrl() {
	return u"https://web.mangogram.org:443/blank.html"_q;
}

// Loaded locally with ExternalShellTopUrl() as the base URI, so that
// the shell document gets the web.mangogram.org origin (required by
// Mini Apps frame-ancestors) without any network requests, that may
// fail in case web.mangogram.org is not accessible from the network.
...
[[nodiscard]] bool IsExternalShellOrigin(const QString &origin) {
	...
	return url.isValid()
		...
		&& url.host() == u"web.mangogram.org"_q
		&& url.port(443) == 443
...
if (window.MangogramGameProxy) {
window.MangogramGameProxy.receiveEvent(
		")"
		+ event.toUtf8()
