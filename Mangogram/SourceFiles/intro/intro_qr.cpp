...
[[nodiscard]] QImage MangogramQrExact(const Qr::Data &data, int pixel) {
	return Qr::Generate(data, pixel, Qt::black);
}

[[nodiscard]] QImage MangogramQr(const Qr::Data &data, int pixel, int max = 0) {
	Expects(data.size > 0);

	if (max > 0 && data.size * pixel > max) {
		pixel = std::max(max / data.size, 1);
	}
	const auto qr = MangogramQrExact(data, pixel * style::DevicePixelRatio());
...
[[nodiscard]] not_null<Ui::RpWidget*> PrepareQrWidget(
	...
	rpl::combine(
		...
		return MangogramQr(code, st::introQrPixel, st::introQrMaxSize);
