...
private:
	...
	void noMangogramCodeDone(const MTPauth_SentCode &result);
	void noMangogramCodeFail(const MTP::Error &result);

	void submitCode(const QString &text);

	void stopCheck();

	object_ptr<Ui::LinkButton> _noMangogramCode;
	mtpRequestId _noMangogramCodeRequestId = 0;
