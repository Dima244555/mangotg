...
, _noMangogramCode(this, tr::lng_code_no_mangogram(tr::now), st::introLink)
, _code(this)
, _callTimer([=] { sendCall(); })
...
, _checkRequestTimer([=] { checkRequest(); }) {
	...
	_noMangogramCode->addClickHandler([=] { noMangogramCode(); });
...
void CodeWidget::refreshLang() {
	if (_noMangogramCode) {
		_noMangogramCode->setText(tr::lng_code_no_mangogram(tr::now));
	}
	updateDescText();
...
void CodeWidget::updateDescText() {
	const auto byMangogram = getData()->codeByMangogram;
	...
	setDescriptionText(!emailPattern.isEmpty()
		...
		? tr::lng_intro_fragment_about(
			...
			tr::rich)
		: (byMangogram ? tr::lng_code_from_mangogram : tr::lng_code_desc)(
			tr::rich));
	if (getData()->codeByMangogram) {
		_noMangogramCode->show();
		_callTimer.cancel();
	} else {
		_noMangogramCode->hide();
		_callStatus = getData()->callStatus;
		_callTimeout = getData()->callTimeout;
...
void CodeWidget::updateControlsGeometry() {
	...
	_noMangogramCode->moveToLeft(contentLeft() + st::buttonRadius, linkTop);
...
void CodeWidget::noMangogramCode() {
	if (_noMangogramCodeRequestId) {
		return;
	}
	_noMangogramCodeRequestId = api().request(MTPauth_ResendCode(
		MTP_flags(0),
		MTP_string(getData()->phone),
		MTP_bytes(getData()->phoneHash),
		MTPstring() // reason
	)).done([=](const MTPauth_SentCode &result) {
		noMangogramCodeDone(result);
	}).fail([=](const MTP::Error &error) {
		noMangogramCodeFail(error);
	}).handleFloodErrors().send();
}

void CodeWidget::noMangogramCodeDone(const MTPauth_SentCode &result) {
	_noMangogramCodeRequestId = 0;
	...
	result.match([&](const MTPDauth_sentCode &data) {
		...
		getData()->codeByMangogram = false;
		updateDescText();
	}, [&](const MTPDauth_sentCodeSuccess &data) {
		finish(data.vauthorization());
	}, [](const MTPDauth_sentCodePaymentRequired &) {
		LOG(("API Error: Unexpected auth.sentCodePaymentRequired "
			"(CodeWidget::noMangogramCodeDone)."));
	});
}

void CodeWidget::noMangogramCodeFail(const MTP::Error &error) {
	_noMangogramCodeRequestId = 0;
	if (MTP::IsFloodError(error)) {
		showCodeError(tr::lng_flood_error());
