...
bool MoveLegacyAlphaFolder(const QString &folder, const QString &file) {
	...
	const auto now = cExeDir() + u"MangogramForcePortable"_q;
...
bool MoveLegacyAlphaFolder() {
	if (!MoveLegacyAlphaFolder(u"MangogramAlpha_data"_q, u"alpha"_q)
		|| !MoveLegacyAlphaFolder(u"MangogramBeta_data"_q, u"beta"_q)) {
		return false;
	}
	return true;
}
...
bool CheckPortableVersionFolder() {
	...
	const auto portable = cExeDir() + u"MangogramForcePortable"_q;
...
void Launcher::init() {
	...
	QApplication::setApplicationName(u"MangogramDesktop"_q);

#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
	// fallback session management is useless for tdesktop since it doesn't have
	// any "are you sure you want to close this window?" dialogs
	// but it produces bugs like https://github.com/mangogramdesktop/tdesktop/issues/5022
	// and https://github.com/mangogramdesktop/tdesktop/issues/7549
	// and https://github.com/mangogramdesktop/tdesktop/issues/948
...
int Launcher::exec() {
	...
	DEBUG_LOG(("Mangogram finished, result: %1").arg(result));
	...
	if (!UpdaterDisabled() && cRestartingUpdate()) {
		...
		DEBUG_LOG(("Sandbox Info: executing Mangogram because of restart."));
