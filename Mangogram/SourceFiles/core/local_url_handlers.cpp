...
bool ResolveUsernameOrPhone(
	...
	if (domain == u"mangogrampassport"_q) {
...
QString TryConvertUrlToLocal(QString url) {
	...
	auto mangogramMeMatch = regex_match(u"^(https?://)?(www\\.)?(mangogram\\.(me|dog)|t\\.me)/(.+)$"_q, url, matchOptions);
	if (mangogramMeMatch) {
		const auto query = mangogramMeMatch->capturedView(5);
...
bool InternalPassportLink(const QString &url) {
	...
	const auto authMatch = regex_match(
		...
		result.command,
		matchOptions);
	const auto authLegacy = (result.username == u"mangogrampassport"_q);
