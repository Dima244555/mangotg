...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"Mangogram"_q : title);
...
, _log(this)
, _close(this) {
	_label.setText(u"Could not start Mangogram Desktop!\nYou can see complete log below:"_q);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Last time Mangogram Desktop was not closed properly."_q);
	} else {
		_label.setText(u"Last time Mangogram Desktop crashed :("_q);
	}
	...
	_getApp.setText(u"GET THE LATEST OFFICIAL VERSION OF MANGOGRAM DESKTOP"_q);
	connect(&_getApp, &QPushButton::clicked, [=] {
		QDesktopServices::openUrl(u"https://desktop.mangogram.org"_q);
	});
...
void LastCrashedWindow::saveReport() {
	QString to = QFileDialog::getSaveFileName(0, u"Mangogram Crash Report"_q, QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + u"/report.mangogramcrash"_q, u"Mangogram crash report (*.mangogramcrash)"_q);
	if (!to.isEmpty()) {
		QFile file(to);
...
void LastCrashedWindow::checkingFinished() {
	...
	if (result == "Old") {
		_pleaseSendReport.setText(u"This report is about some old version of Mangogram Desktop."_q);
		_sendingState = SendingTooOld;
		updateControls();
		return;
	} else if (result == "Unofficial") {
		_pleaseSendReport.setText(u"You use some custom version of Mangogram Desktop."_q);
	...
	reportPart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"report\"; filename=\"report.mangogramcrash\""));
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding + QFontMetrics(_label.font()).horizontalAdvance(u"Last time Mangogram Desktop was not closed properly."_q) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
