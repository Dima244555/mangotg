...
[[nodiscard]] bool IsMangogramShortLinkHost(const QUrl &url) {
	using namespace qthelp;

	return regex_match(
		"(^|\\.)(mangogram\\.(me|dog)|t\\.me)$",
		url.host(),
		RegExOption::CaseInsensitive).valid();
}

[[nodiscard]] bool HiddenUrlRequiresConfirmation(const QUrl &url) {
	return UrlRequiresConfirmation(url) || IsMangogramShortLinkHost(url);
}
...
void ExportToCalendar(TimeId date, const QString &messageText) {
	...
	const auto content = u"BEGIN:VCALENDAR\r\n"
		"VERSION:2.0\r\n"
		"PRODID:-//Mangogram Desktop//EN\r\n"
		"BEGIN:VEVENT\r\n"
		"DTSTART:%1\r\n"
		"DTEND:%2\r\n"
		"DTSTAMP:%3\r\n"
		"UID:mangogram-%4-%7@mangogram.org\r\n"
		"SUMMARY:%5\r\n"
		"DESCRIPTION:%6\r\n"
		"END:VEVENT\r\n"
		"END:VCALENDAR\r\n"_q
