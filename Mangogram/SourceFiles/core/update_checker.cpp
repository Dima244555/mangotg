...
bool checkReadyUpdate() {
	...
	QFileInfo updater(cWorkingDir() + u"tupdates/temp/Mangogram.app/Contents/Frameworks/Updater"_q);
#else // Q_OS_MAC
...
void UpdateApplication() {
	if (UpdaterDisabled()) {
		const auto url = [&] {
#ifdef OS_WIN_STORE
			return "https://www.microsoft.com/en-us/store/p/mangogram-desktop/9nztwsqntd0s";
#elif defined OS_MAC_STORE // OS_WIN_STORE
			return "https://itunes.apple.com/ae/app/mangogram-desktop/id946399090";
#else // OS_WIN_STORE || OS_MAC_STORE
			if (KSandbox::isFlatpak()) {
				return "https://flathub.org/apps/details/org.mangogram.desktop";
			} else if (KSandbox::isSnap()) {
				return "https://snapcraft.io/mangogram-desktop";
			}
			return "https://desktop.mangogram.org";
...
QString countAlphaVersionSignature(uint64 version) { // duplicated in packer.cpp
	...
	QByteArray signedData = (qstr("MangogramBeta_") + QString::number(version, 16).toLower()).toUtf8();
