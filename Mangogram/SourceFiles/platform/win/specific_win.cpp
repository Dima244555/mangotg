...
#ifdef OS_WIN_STORE
		return appData.absolutePath() + u"/Mangogram Desktop UWP/"_q;
#else // OS_WIN_STORE
...
void psDoFixPrevious() {
	try {
		...
		if (SUCCEEDED(userDesktopRes)) {
			userDesktopLnk = QString::fromWCharArray(userDesktopFolder) + "\\Mangogram.lnk";
		}
		if (SUCCEEDED(commonDesktopRes)) {
			commonDesktopLnk = QString::fromWCharArray(commonDesktopFolder) + "\\Mangogram.lnk";
		}
...
void AutostartToggle(bool enabled, Fn<void(bool)> done) {
	...
	const auto success = ManageAppLink(
		...
		FOLDERID_Startup,
		L"-autostart",
		L"Mangogram autorun link.\n"
		"You can disable autorun in Mangogram settings.");
	if (done) {
...
void psSendToMenu(bool send, bool silent) {
	ManageAppLink(
		...
		FOLDERID_SendTo,
		L"--",
		L"Mangogram send to link.\n"
		"You can disable send to menu item in Mangogram settings.");
}
