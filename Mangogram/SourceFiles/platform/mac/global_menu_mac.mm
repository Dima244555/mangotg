...
struct ComputedState {
	...
	bool showMangogramDisabled = false;
...
private:
	...
	QAction *_showMangogram = nullptr;
...
void Manager::retranslate() {
	...
	if (_showMangogram) {
		_showMangogram->setText(tr::lng_mac_menu_show(tr::now));
	}
	if (_fullScreen) {
...
void Manager::recomputeState() {
	...
	auto next = ComputedState{
		...
		.showMangogramDisabled = widget->isActive(),
	...
	ForceDisabled(_showMangogram, next.showMangogramDisabled);
...
void Manager::buildWindowMenu(QMenu *window) {
	...
	window->addSeparator();
	_showMangogram = window->addAction(
		u"Show Mangogram"_q,
		receiver,
		[this] {
...
void Manager::buildMenu() {
	buildAppleMenu(_menuBar->addMenu(u"Mangogram"_q));
...
void Manager::destroy() {
	...
	_logout = _undo = _redo = _cut = _copy = _paste = _delete
		...
		= _newChannel = _showMangogram = _fullScreen = _emoji
