...
bool GenerateDesktopFile(
	...
	const auto sourceFile = u":/misc/org.mangogram.desktop.desktop"_q;
	...
	if (!Core::UpdaterDisabled()) {
		...
		QFile::remove(u"%1mangogram.desktop"_q.arg(targetPath));
		QFile::remove(u"%1mangogramdesktop.desktop"_q.arg(targetPath));
		...
		if (!Core::Launcher::Instance().customWorkingDir()) {
			QFile::remove(u"%1org.mangogram.desktop._%2.desktop"_q.arg(
				targetPath,
				md5Hash));
		...
		QFile::remove(u"%1org.mangogram.desktop.%2.desktop"_q.arg(
			targetPath,
			md5Hash));
	}
...
bool GenerateServiceFile(bool silent = false) {
	...
	if (!Core::UpdaterDisabled()
		...
		QFile::remove(u"%1org.mangogram.desktop._%2.service"_q.arg(
			targetPath,
			md5Hash));
	}
...
void InstallLauncher() {
	...
	QFile::remove(icon);
	QFile::remove(icons + u"mangogram.png"_q);
...
QString psAppDataPath() {
	// Previously we used ~/.MangogramDesktop, so look there first.
	// If we find data there, we should still use it.
	auto home = QDir::homePath();
	if (!home.isEmpty()) {
		auto oldPath = home + u"/.MangogramDesktop/"_q;
...
void start() {
	QGuiApplication::setDesktopFileName([&] {
		...
		if (!Core::UpdaterDisabled()) {
			return u"org.mangogram.desktop._%1"_q.arg(
				Core::Launcher::Instance().instanceHash().constData());
		}

		return u"org.mangogram.desktop"_q;
	}());
