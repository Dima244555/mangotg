...
, _mangogram(
	Ui::CreateChild<Ui::FlatLabel>(_footer.get(), st::mainMenuMangogramLabel))
, _version(AddVersionLabel(_footer)) {
	...
	_footer->heightValue(
		...
		_mangogram->moveToLeft(st::mainMenuFooterLeft, _footer->height() - st::mainMenuMangogramBottom - _mangogram->height());
	...
	_mangogram->setMarkedText(tr::link(
		u"Mangogram Desktop"_q,
		u"https://desktop.mangogram.org"_q));
	_mangogram->setLinksTrusted();
