...
, _singleMessageSearch(&controller->session()) {
	...
	_search->setMimeDataHook([=](
		...
		if (data->hasFormat(u"application/x-mangogram-dialog"_q)) {
...
void Widget::checkUpdateStatus() {
	...
	if (Checker().state() == Checker::State::Ready) {
		...
		_updateMangogram.create(
			this,
			tr::lng_update_mangogram(tr::now),
			st::dialogsUpdateButton,
			st::dialogsInstallUpdate,
			st::dialogsInstallUpdateOver,
			true);
		_updateMangogram->show();
		_updateMangogram->setClickedCallback([] {
...
void Widget::updateControlsGeometry() {
	...
	putBottomButton(_updateMangogram);
