...
This is a testbench repository for the tgcalls VoIP library (from Mangogram). It contains the full Mangogram iOS source tree as a build dependency, but the focus is on testing and debugging tgcalls.
...
```bash
# Launch 1000 tasks across all Mangogram reflectors, 30s each
...
# Reflector mode (routes through a real Mangogram UDP reflector)
...
- **Reflector**: Routes through a Mangogram UDP reflector, `enableP2P=false`, configures `RtcServer` with `login="reflector"` and random peer tags (16 bytes, byte 0 = `0x00` for caller, `0x01` for callee)
